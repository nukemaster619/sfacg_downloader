...
charMap = {source: target for source, target in zip(SPECIAL_CHARS, REPLACEMENT_CHARS)}
...
STRINGS = {
    "zh": {
        "select_lang": "请选择界面语言 / Please select your language",
        "lang_menu": "1. 中文\n2. English",
        "lang_prompt": "输入选项 (默认 1): ",
        "invalid_lang": "无效选项，已默认使用中文。",
        "bootstrapping": "正在初始化请求签名...",
        "bootstrapped": "初始化完成。",
        "cookie_created": "cookie.txt 已创建",
        "cookie_read": "已读取 cookie:",
        "input_phone": "输入手机号: ",
        "input_password": "输入密码: ",
        "login_failed": "登录失败，请检查手机号或密码。",
        "input_novel_id": "输入小说ID: ",
        "title_fetch_failed": "获取标题失败",
        "catalog_fetch_failed": "获取目录失败",
        "cover_fetch_failed": "封面下载失败，将继续生成无封面的 EPUB。",
        "novel_title": "小说标题",
        "downloading_volume": "正在下载分卷:",
        "downloaded": "已下载",
        "chapter_locked": "下载失败，请确认该章节已经购买或解锁",
        "chapter_network_fail": "下载失败，可能是网络问题",
        ...
        "fatal": "程序发生错误:",
        "retrying": "请求返回 417，正在重新签名后重试...",
        "catalog_empty": "没有获取到目录，或者目录为空。",
        "choose_volume": "可下载分卷如下：",
        "checking_login": "正在检查登录状态...",
        "login_ok": "登录成功。",
        "logging_in": "正在登录...",
        "fetching_catalog": "正在获取书籍信息和目录...",
        "fetching_cover": "正在下载封面...",
        "image_embed_failed": "插图嵌入失败",
        "saving_cookie": "正在保存 cookie...",
        "loading_cookie": "正在读取本地 cookie...",
        "skipping_volume": "跳过分卷:",
        "volume_done": "分卷处理完成:",
        "chapters_done": "章节处理完成，共成功下载",
        "empty_selection": "没有选中任何有效分卷",
        "using_saved_cookie": "尝试使用本地 cookie 登录...",
        "missing_content": "章节没有返回正文内容",
        "cover_done": "封面下载完成",
        "book_building": "正在生成电子书...",
        "book_writing": "正在写入文件...",
    },
    "en": {
        "select_lang": "请选择界面语言 / Please select your language",
        "lang_menu": "1. 中文\n2. English",
        "lang_prompt": "Choose option (default 1): ",
        "invalid_lang": "Invalid option. Defaulting to Chinese.",
        "bootstrapping": "Preparing request signature...",
        "bootstrapped": "Initialization complete.",
        "cookie_created": "cookie.txt created",
        "cookie_read": "Loaded cookie:",
        "input_phone": "Enter phone number: ",
        "input_password": "Enter password: ",
        "login_failed": "Login failed. Please check the phone number or password.",
        "input_novel_id": "Enter novel ID: ",
        "title_fetch_failed": "Failed to fetch title",
        "catalog_fetch_failed": "Failed to fetch catalog",
        "cover_fetch_failed": "Cover download failed. EPUB will be created without a cover.",
        "novel_title": "Novel title",
        "downloading_volume": "Downloading volume:",
        "downloaded": "downloaded",
        "chapter_locked": "download failed, make sure this chapter has been purchased or unlocked",
        "chapter_network_fail": "download failed, possibly due to network issues",
        ...
        "fatal": "Program error:",
        "retrying": "Request returned 417, refreshing signature and retrying...",
        "catalog_empty": "No catalog was returned, or the catalog is empty.",
        "choose_volume": "Available volumes:",
        "checking_login": "Checking login state...",
        "login_ok": "Login successful.",
        "logging_in": "Signing in...",
        "fetching_catalog": "Fetching novel details and catalog...",
        "fetching_cover": "Downloading cover...",
        "image_embed_failed": "Image embed failed",
        "saving_cookie": "Saving cookie...",
        "loading_cookie": "Reading local cookie...",
        "skipping_volume": "Skipping volume:",
        "volume_done": "Finished volume:",
        "chapters_done": "Finished chapter batch, downloaded",
        "empty_selection": "No valid volume was selected",
        "using_saved_cookie": "Trying saved cookie first...",
        "missing_content": "No chapter body was returned",
        "cover_done": "Cover downloaded",
        "book_building": "Building ebook...",
...
def choose_language() -> None:
    ...
    print(STRINGS["zh"]["select_lang"])
    print(STRINGS["zh"]["lang_menu"])
    choice = input(STRINGS["zh"]["lang_prompt"]).strip()
    ...
    if choice in ("", "1"):
        ...
        safe_print(STRINGS["zh"]["invalid_lang"])
        LANG = "zh"
...
def get_sign(nonce: str, timestamp: int, device_token: str) -> str:
    ...
    for index in range(101):
        mixed_chars.append(chr((auth_string[index] + nonce_reorder[index]) >> 1))

    result = "".join(mixed_chars)
...
class SfacgClient:
        ...
        *,
        ...
        for attempt in range(1, retries + 1):
            try:
                ...
                if http_code == 417 and allow_417_retry and attempt < retries:
                    safe_print(tr("retrying"))
    ...
    def post_json(
        self, url: str, payload: dict, *, retries: int = DEFAULT_RETRIES, timeout: int = REQUEST_TIMEOUT
    ) -> Tuple[requests.Response, dict]:
    ...
    def fetch_catalog(self, novel_id: str) -> NovelCatalog:
        ...
        for index, volume_data in enumerate(volume_list, start=1):
            ...
            volumes.append(
                Volume(
                    index=index,
                    title=volume_data.get("title", f"Volume {index}"),
    ...
    def fetch_chapter(self, chapter_ref: ChapterRef) -> Optional[ChapterContent]:
        ...
        extra_content = body.get("expand", {}).get("content", "")
        combined = f"{main_content}{extra_content}"

        if not combined:
            safe_print(f"{title}: {tr('missing_content')}")
            combined = ""

        text = "".join(charMap.get(char, char) for char in combined)
    ...
    def fetch_binary(self, url: str, *, retries: int = 3, timeout: int = REQUEST_TIMEOUT) -> bytes:
        ...
        if last_error is not None:
            raise last_error
        raise RuntimeError("Binary download failed without an explicit error")
...
def ensure_login(client: SfacgClient) -> None:
    ...
    while True:
        ...
        if cookie != "error" and client.check_login():
            safe_print(tr("login_ok"))
            return
...
def chapter_to_xhtml(
    ...
    image_cache: Dict[str, Tuple[str, str, bytes]],
    ...
    for raw_line in chapter.content.splitlines():
        ...
        if match:
            ...
            if cached:
                ...
                parts.append(f'<img src="{html.escape(src)}" alt="{html.escape(filename)}"/>')
            else:
...
def build_epub_and_txt(
    ...
    for volume in catalog.volumes:
        ...
        image_urls = collect_unique_image_urls(chapter_results)
        image_cache = download_images(client, image_urls)
...
def main() -> None:
    ...
    catalog = client.fetch_catalog(novel_id)
